...
local function confirm(question, on_confirm)
    ...
    Snacks.input({
        prompt = text,
        win = {
            width = math.min(math.max(#text + 20, 40), vim.o.columns - 4),
            keys = {
...
-- ---------------------------------------------------------------
-- commit action picker
--
-- same shape as branch/stash action pickers: verb chosen by
-- keymap, picker scoped to that one action, fires on <CR>.
-- ---------------------------------------------------------------

local function commit_action_picker(root, action)
    local actions = require("telescope.actions")
    local state = require("telescope.actions.state")
...
--
-- lists every file that exists at `ref` (branch or commit), with
-- a live diff against your current working copy so you can see
-- exactly what pulling it in would change. <CR> runs the restore
-- and reloads the buffer if it's open.
-- ---------------------------------------------------------------
    ...
    pickers.new({
        ...
        attach_mappings = function(_, map)
            actions.select_default:replace(function(bufnr)
                ...
                confirm("Checkout '" .. path .. "' from " .. ref .. "?", function()
...
local function checkout_file_from_branch(root)
    branch_action_picker(root, function(branch)
        checkout_file_picker(root, branch)
    end)
end

local function checkout_file_from_commit(root)
    commit_action_picker(root, function(hash)
        checkout_file_picker(root, hash)
    end)
end
...
local function discard_file(root)
    ...
    confirm("Discard all changes to '" .. rel .. "'?", function()
